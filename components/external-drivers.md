...
SliderMC pinout: [PINS.md](../mc/pins.md) · [MC_Pico_pinout.png](../assets/img/MC_Pico_pinout.png) · [CONFIG.md](../mc/config.md) · [ARCHITECTURE.md](../architecture/overview.md)
...
**Config (SliderMC `mc.ini` / `CS`):** e.g. `steps_per_unit_1=320` for 200 full steps × 8 microsteps / 5 mm/rev; `DRV_EN_1_active=0`.
...
JKSlider / SliderMC do **not** bit-bang SPI. Program `MRES` / current with another host so **microsteps** match `steps_per_unit_1`, then run STEP/DIR from the **SliderMC** Pico.
