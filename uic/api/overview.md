...
**User hardware profile:** copy [`SliderPins.example.py`](https://github.com/fablab-wue/SliderCtrl/blob/main/SliderPins.example.py) → `SliderPins.py` and edit **that file only**.
...
Wiring and menu: [../../components/mks-servoxx.md](../../components/mks-servoxx.md).
...
Example screens (`../../assets/img/oled/` — flat active-area mockups; regenerate with `python tools/oled/render_examples.py`). Panel extras (`Delay` / `Wait` / `TL` / remain) come from `JKSlider` via `setOledText()`.
...
**For your hardware, copy `SliderPins.example.py` → `SliderPins.py` and edit that file only** (pins + behaviour). Axis STEP/DIR/EN, home, and DRV_ERROR live on **SliderMC** ([PINS.md](../../mc/pins.md) / [CONFIG.md](../../mc/config.md)).
