...
For wiring, panel variants, and config files, see [technical/README.md](technical/README.md) ([Panel](technical/panel.md), [Config](technical/config.md)).  
...
PWM RGB and optional NeoPixel (WS2812) show the **same** colors. Soft-limit blues are **added** onto the motion/idle base (shared [`UIC_Base`](https://github.com/fablab-wue/SliderCtrl/blob/main/UIC_base.py)); Delay / TL / loop panel colors come from the app via `ledPingPong` / `ledAddColor`. See [API — RGB status LED](../../api/overview.md#rgb-status-led-led_r--led_g--led_b--optional-neopixel).
