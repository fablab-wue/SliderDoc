...
<style>
:root {
  --doc-title: "JKSlider — Technical Manual: Config";
  --doc-path: ".\\SliderDoc\\uic\\projects\\jkslider\\technical\\config.md";
}
</style>
...
Session vs persistent MC keys: [PROTOCOL — Session vs config](../../../../contract/protocol.md#session-vs-config) · [CONFIG.md keys](../../../../mc/config.md#keys).
...
STEP/DIR/EN, SW_HOME, DRV_ERROR, and hard limits are on the **SliderMC** Pico — see [PINS.md](../../../../mc/pins.md) and [pico_pinout_mc.png](../../../../assets/img/pico_pinout_mc.png).

**RP2040-Zero as UIC** (button mode): copy the `RP2040_ZERO_*` keys from `SliderPins.example.py` into `SliderPins.py`. UART GP12/13, I2C1 GP14/15, pinout [rp2040zero_pinout_button.png](../../../../assets/img/rp2040zero_pinout_button.png). Details: [panel.md](panel.md#rp2040-zero-uic-button-mode).
...
For your hardware, copy [`SliderPins.example.py`](https://github.com/fablab-wue/SliderCtrl/blob/main/SliderPins.example.py) → `SliderPins.py` and edit **that file only** (pins **and** behaviour). Keep one profile per physical slider build.
...
- RGB LED wiring: see [Wiring schematics — RGB LED](panel.md#wiring-schematics--rgb-led)
...
- OLED mockups for docs: `python tools/oled/render_examples.py` → `../../../../assets/img/oled/*.png`
