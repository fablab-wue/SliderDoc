...
**Related:** [config.md](config.md) · [motion.md](motion.md) · [protocol — Optional 2nd axis](../contract/protocol.md#optional-2nd-axis-axis2_use) · [pins.md](pins.md)
...
(clamped to `max_speed` / `max_accel`; see `motion_move_to2` in SliderMC `planner.cpp`).

- Axis-2-only (`MT _ b` / `MT none b`) uses full `SS`/`SA` on axis 2.
...
This is **not** a dedicated sync-stop controller — only matching per-axis physics. Sync can break if `max_speed` / `max_accel` clamped axis 2, one axis was already braking for its target, or the move was jog / single-axis (no dual scaling).
...
**Shared** (not per-axis): session `SS`/`SA`, `max_speed`/`max_accel`, `unit_name`, `axis2_use`, `name`, ramp/path globals, debug/verbose init.
...
1. **Skip tokens:** `MT _ 45` / `MT none 45` for pan-only; `MT 200` stays axis1-only.
2. **Jog masks:** `ML` / `MR` with `0|1|2` — which axes jog (`0` = both when axis2 is on).
3. **Prefer pan on axis 2** — keeps linear “mm” on axis 1 (`IP` first field / UIC habit).
4. **`max_speed` clamp:** if `|d2| ≫ |d1|`, scaled `v2` may hit `max_speed` and **lose** perfect time sync — shorten the axis2 move, raise `max_speed`, or move axes sequentially. Mid-move `SS`/`SA` stay time-synced while coordination is active (same clamp still applies).
5. **Path mode (`PG`):** dual `PD` is **slice-timed**, not the same as dual-`MT` distance scaling. See [motion-path.md](motion-path.md).
6. **UIC:** JKSlider / B4Slider UIs are still mostly 1-axis; dual `MT` is driven by hosts/scripts. Use `CG unit_name` for the display unit.
...
- [ ] Watch `|d2|/|d1|` vs `max_speed` on dual seeks
