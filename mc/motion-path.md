...
alongside the normal sine-ramp planner (`MT`/`MB`/`MJ`/`MH`) — see
...
checks `max_speed_N`/`max_accel_N`. That is the right model for interactive jogs
...
1. Converts the signed µm distance to a step count using `steps_per_unit_N`.
...
- **Steps:** `steps_f = distance_mm * steps_per_unit_N + step_err_carry`;
...
- **Speed and acceleration limits** (`max_speed_N`/`max_accel_N`) are
