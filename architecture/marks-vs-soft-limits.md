...
**What it does.** The working window is the **allowed stretch/shrink of rail**. You set a left end and a right end. Those ends are both a **wall** (you cannot leave) and the **MOVE target** (MOVE_L cruises to the left end and stops). There is no separate “go to A” key — left and right **are** A/B in the manual.
