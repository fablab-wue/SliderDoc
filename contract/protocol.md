...
Important keys: `init_speed`, `init_accel`, `max_speed_1`, `max_accel_1`, `max_speed_2`, `max_accel_2`, `max_speed_3`, `max_accel_3`, `steps_per_unit_1`, `motor_N_unit` / `servo_N_unit` (stored; defaults `mm` / `deg`), `MOTOR_N_min`/`MOTOR_N_max`, `SERVO_N_min`/`SERVO_N_max`, `SERVO_N_min_pulse`/`SERVO_N_max_pulse`/`SERVO_N_swap`, synthesized `axis_min_N`/`axis_max_N` and `axis_N_unit` (packed copy of the motor or servo label), `motors`, `servos`, `axis` (read-only sum; dump emits it), `name`, `init_verbose`, `init_terminal`, `init_debug_level`, pin keys with the digit **before** the suffix (`DRV_STEP_1_active`, `SW_LIMIT_R_3_use`), `BUZZER_use`, `home_mode_1` / `home_move_out_1` / `home_speed_1` / `home_accel_1`, matching `_2` / `_3` keys, `ramp_start_hz`, `stop_approach_hz`, `dir_change_pause_s`. Synonyms: `steps_per_mm_N`, `soft_min_N` / `soft_max_N` (aliases of packed `axis_min_N`). **`CS axis`, `CS axis_N_unit`, and `CS slider_*` are rejected.** See [config.md](../mc/config.md).
